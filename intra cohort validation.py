...
def plot_calibration(y_true, y_proba, title, save_path):
    ...
    # Using 5 bins instead of 10 because your test cohorts are small
...
def run_inter_cohort(train_data, test_data, train_name, test_name, n_feat, feat_type):
    ...
    # --- NEW: Dynamic, SHUFFLED CV Strategy to prevent 1-class folds ---
    y_train_flat = y_train.values.ravel()
    min_class_count = np.min(np.bincount(y_train_flat.astype(int)))

    # Ensure we don't request more folds than we have minority samples (max 5)
    ...
    for clf_name, (clf, params) in CLASSIFIERS.items():
        ...
        # A. NO FUSION
        # ---------------------------------
        # Notice we replaced cv=5 with cv=cv_outer
        ...
        results_rows.append({'Train_Cohort': train_name, 'Test_Cohort': test_name, 'Model': clf_name, 'Fusion': 'None',
                             'Features': n_feat, 'Selection': feat_type, **stats_none})
        ...
        results_rows.append({'Train_Cohort': train_name, 'Test_Cohort': test_name, 'Model': clf_name, 'Fusion': 'Early',
                             'Features': n_feat, 'Selection': feat_type, **stats_ef})
        ...
        # Inner CV needs its own dynamic limit
        ...
        if train_proba_list:
            ...
            results_rows.append(
                {'Train_Cohort': train_name, 'Test_Cohort': test_name, 'Model': clf_name, 'Fusion': 'Late',
                 'Features': n_feat, 'Selection': feat_type, **stats_lf})
...
def calculate_p_value(dist_a, dist_b):
    # Find the smallest length between the two
...
def generate_comparisons(df_results):
    # [Unchanged from your original code]
    comparison_rows = []
    groups = df_results.groupby(['Train_Cohort', 'Test_Cohort', 'Model', 'Selection'])

    for name, group in groups:
        train, test, model, sel = name
        baseline_feat = 14
        base_rows = group[group['Features'] == baseline_feat]

        if not base_rows.empty:
            for fusion in ['None', 'Early', 'Late']:
                base_row = base_rows[base_rows['Fusion'] == fusion]
                if base_row.empty: continue
                base_dist, base_mean = base_row.iloc[0]['raw_aucs'], base_row.iloc[0]['Mean_AUC']

                others = group[(group['Features'] != baseline_feat) & (group['Fusion'] == fusion)]
                for _, row in others.iterrows():
                    curr_dist, curr_mean = row['raw_aucs'], row['Mean_AUC']
                    comparison_rows.append({
                        'Train': train, 'Test': test, 'Model': model, 'Selection': sel,
                        'Comparison': f'{row["Features"]} vs 14 (Feats)', 'Context': fusion,
                        'Diff_Mean_AUC': curr_mean - base_mean,
                        'p_value_t': calculate_p_value(curr_dist, base_dist),
                        'Significant': 'Yes' if calculate_p_value(curr_dist, base_dist) < 0.05 else 'No'
                    })

        for feat in group['Features'].unique():
            rows_feat = group[group['Features'] == feat]

            def get_dist(fus_type):
                r = rows_feat[rows_feat['Fusion'] == fus_type]
                if r.empty: return None, None
                return r.iloc[0]['raw_aucs'], r.iloc[0]['Mean_AUC']

            dist_none, mean_none = get_dist('None')
            dist_early, mean_early = get_dist('Early')
            dist_late, mean_late = get_dist('Late')

            comparisons = [('Early', 'Late', dist_early, mean_early, dist_late, mean_late),
                           ('Early', 'None', dist_early, mean_early, dist_none, mean_none),
                           ('Late', 'None', dist_late, mean_late, dist_none, mean_none)]

            for tag_a, tag_b, dist_a, mean_a, dist_b, mean_b in comparisons:
                if dist_a is not None and dist_b is not None:
                    p_val = calculate_p_value(dist_a, dist_b)
                    comparison_rows.append({
                        'Train': train, 'Test': test, 'Model': model, 'Selection': sel,
                        'Comparison': f'{tag_a} vs {tag_b}', 'Context': f'{feat} Feats',
                        'Diff_Mean_AUC': mean_a - mean_b, 'p_value_t': p_val,
...
df_main_clean = df_main.drop(columns=['raw_aucs'])
