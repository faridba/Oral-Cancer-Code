...
def get_qmids_bootstrapped_metrics(y_true, qmics_proba, n_boot=1000):
    """Calculates bootstrapped AUC distribution for the QMIDS baseline."""
...
def run_inter_cohort(train_data, test_data, train_name, test_name, n_feat, feat_type):
    ...
    # Get QMIDS baseline bootstrapped metrics
...
def calculate_p_value(dist_a, dist_b):
    """Calculates a TWO-SIDED empirical p-value from bootstrap distributions."""
    ...
    # Calculate the differences between the two bootstrap distributions
    diffs = a_arr - b_arr

    # Calculate the proportion of differences below 0 and above 0
    p_lower = np.mean(diffs <= 0)
    p_upper = np.mean(diffs >= 0)

    # A two-sided empirical p-value is 2 * the smaller of the two tails
    p_two_sided = 2 * min(p_lower, p_upper)

    # Ensure the p-value mathematically cannot exceed 1.0
    return min(p_two_sided, 1.0)


def generate_comparisons(df_results):
    """Generates Early vs qMIDS, Late vs qMIDS, and Early vs Late comparisons."""
    comparison_rows = []
    groups = df_results.groupby(['Train_Cohort', 'Test_Cohort', 'Model', 'Selection', 'Features'])

    for name, group in groups:
        train, test, model, sel, feats = name

        early_row = group[group['Fusion'] == 'Early']
        late_row = group[group['Fusion'] == 'Late']

        if early_row.empty or late_row.empty:
            continue

        early_dist = early_row.iloc[0]['raw_aucs']
        early_mean = early_row.iloc[0]['Mean_AUC']

        late_dist = late_row.iloc[0]['raw_aucs']
        late_mean = late_row.iloc[0]['Mean_AUC']

        qmids_dist = early_row.iloc[0]['qmids_raw_aucs']
        qmids_mean = early_row.iloc[0]['qmids_Mean_AUC']

        # 1. Early Fusion vs qMIDS
        p_val_early_qmids = calculate_p_value(early_dist, qmids_dist)
        comparison_rows.append({
            'Train_Cohort': train, 'Test_Cohort': test, 'Model': model, 
            'Selection': sel, 'Features': feats, 'Comparison': 'Early vs qMIDS',
            'Model_A_Mean_AUC': early_mean, 'Model_B_Mean_AUC': qmids_mean,
            'Diff_Mean_AUC': early_mean - qmids_mean, 'p_value_bootstrap': p_val_early_qmids,
            'Significant': 'Yes' if p_val_early_qmids < 0.05 else 'No'
        })

        # 2. Late Fusion vs qMIDS
        p_val_late_qmids = calculate_p_value(late_dist, qmids_dist)
        comparison_rows.append({
            'Train_Cohort': train, 'Test_Cohort': test, 'Model': model, 
            'Selection': sel, 'Features': feats, 'Comparison': 'Late vs qMIDS',
            'Model_A_Mean_AUC': late_mean, 'Model_B_Mean_AUC': qmids_mean,
            'Diff_Mean_AUC': late_mean - qmids_mean, 'p_value_bootstrap': p_val_late_qmids,
            'Significant': 'Yes' if p_val_late_qmids < 0.05 else 'No'
        })

        # 3. Early Fusion vs Late Fusion
        p_val_early_late = calculate_p_value(early_dist, late_dist)
        comparison_rows.append({
            'Train_Cohort': train, 'Test_Cohort': test, 'Model': model, 
            'Selection': sel, 'Features': feats, 'Comparison': 'Early vs Late',
            'Model_A_Mean_AUC': early_mean, 'Model_B_Mean_AUC': late_mean,
            'Diff_Mean_AUC': early_mean - late_mean, 'p_value_bootstrap': p_val_early_late,
            'Significant': 'Yes' if p_val_early_late < 0.05 else 'No'
        })
